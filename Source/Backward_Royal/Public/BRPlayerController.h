...
public:
  ...
  void CreateRoomWithPlayerName(const FString &RoomName,
  ...
  /** 로비에서 방 나가기. 클라이언트는 서버 연결을 끊고, 호스트는 세션을 종료한
   * 뒤 메인 맵으로 이동합니다. */
  UFUNCTION(BlueprintCallable, Category = "Session")
  void LeaveRoom();
  ...
  /** 로비: Entry → SelectTeam 슬롯 배치 요청 (자신을 TeamIndex 팀의 SlotIndex
   * 슬롯에 배치). TeamIndex 0~3=팀1~4, SlotIndex 0=1Player 1=2Player */
  UFUNCTION(BlueprintCallable, Category = "Lobby")
  void RequestAssignToLobbyTeam(int32 TeamIndex, int32 SlotIndex);

  /** 로비: SelectTeam 슬롯 → Entry로 이동 요청 (해당 팀/슬롯의 플레이어를 Entry
   * 첫 빈 자리로) */
  UFUNCTION(BlueprintCallable, Category = "Lobby")
  void RequestMoveToLobbyEntry(int32 TeamIndex, int32 SlotIndex);

  /** 로비: 자신이 현재 있는 팀 슬롯을 찾아 대기열(Entry)로 이동 요청. 팀에
   * 없으면 아무 동작 안 함 */
  UFUNCTION(BlueprintCallable, Category = "Lobby")
  ...
  /** PIE/서버 종료 시 위젯 참조를 먼저 끊을 때 호출 (GameInstance
   * DoPIEExitCleanup에서 호출) */
...
protected:
  ...
  // [ServerSetProfile 복구]
  UFUNCTION(Server, Reliable)
  void ServerSetProfile(const FBaseUserInfo &InProfile);
