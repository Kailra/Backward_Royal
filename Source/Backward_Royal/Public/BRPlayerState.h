...
DECLARE_DYNAMIC_MULTICAST_DELEGATE_OneParam(FOnPlayerRoleChanged, bool,
                                            bIsLowerBody);
DECLARE_DYNAMIC_MULTICAST_DELEGATE_OneParam(FOnPlayerStatusChanged,
...
public:
  ...
  UPROPERTY(ReplicatedUsing = OnRep_TeamNumber, BlueprintReadOnly,
            Category = "Team")
  int32 TeamNumber;

  // 방장 여부
  UPROPERTY(ReplicatedUsing = OnRep_IsHost, BlueprintReadOnly,
            Category = "Room")
  bool bIsHost;
  ...
  // 영구 유저 프로필(이름, UID, 레벨 등) - 게임 인스턴스에서 동기화됨
  UPROPERTY(ReplicatedUsing = OnRep_MyProfile, BlueprintReadWrite,
            Category = "User Info")
  ...
  // 프로필 업데이트 (서버 전용)
  UFUNCTION(BlueprintCallable, Category = "User Info")
  void SetPlayerProfile(const FBaseUserInfo &InProfile);
  ...
  /** [서버 전용] 유저 인포 관련 값이 바뀌었을 때 GameState의
   * PlayerListForDisplay 갱신 요청 */
  void NotifyUserInfoChanged();

  UPROPERTY(ReplicatedUsing = OnRep_PlayerStatus, BlueprintReadOnly,
            Category = "Status")
  ...
  // [Refactored] CustomizationData 삭제됨 -> MyProfile.UserCustomData 사용

  // 구독 가능한 이벤트 디스패처
  UPROPERTY(BlueprintAssignable, Category = "Events")
  FOnCustomizationDataChanged OnCustomizationDataChanged;

  // 서버에 내 커마 정보를 알리는 함수 (삭제 예정 - ServerSetProfile로 통합
  // 권장하거나, MyProfile 업데이트로 변경)
  UFUNCTION(Server, Reliable)
  void ServerSetCustomizationData(const FBRCustomizationData &NewData);

  // MyProfile.UserCustomData가 변경되면 통합 OnRep_MyProfile이 호출되므로 별도
  // OnRep 삭제 가능 다만 하위 호환성을 위해 함수만 남겨두거나 삭제. 여기선
  // 삭제.
