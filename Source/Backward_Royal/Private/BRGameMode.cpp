...
void ABRGameMode::BeginPlay() {
  ...
  // 로비에서 랜덤 팀 배정 후 예약된 경우: 게임 맵 로드 후 플레이어 스폰이 끝날
  // 때까지 지연 후 적용 플래그는 ApplyRoleChangesForRandomTeams에서만 클리어
  // (여기서 지우면 1.5초 후 타이머에서 진입 시 플래그가 false라 적용이 스킵됨)
  if (UBRGameInstance *GI = Cast<UBRGameInstance>(GetGameInstance())) {
    if (GI->GetPendingApplyRandomTeamRoles()) {
      FTimerHandle H;
      GetWorld()->GetTimerManager().SetTimer(
          H, this, &ABRGameMode::ApplyRoleChangesForRandomTeams, 1.5f, false);
      UE_LOG(LogTemp, Log,
             TEXT("[랜덤 팀 적용] 게임 맵 로드됨 - 1.5초 후 상체/하체 Pawn "
                  "적용 예정"));
    }
  }
}
...
void ABRGameMode::PostLogin(APlayerController *NewPlayer) {
  ...
  if (BRPS && BRGameState) {
    ...
    UE_LOG(LogTemp, Warning,
           TEXT("[로비이름] PostLogin 진입 | bIsLocalPlayer 판단 전 | "
                "GetPlayerName()='%s'"),
           *PlayerName);

    if (PlayerName.IsEmpty()) {
      PlayerName =
          FString::Printf(TEXT("Player %d"), BRGameState->PlayerArray.Num());
      UE_LOG(
          LogTemp, Warning,
          TEXT("[로비이름] PostLogin | PlayerName 비어있어 기본값 사용: '%s'"),
          *PlayerName);
    }

    // GameInstance 이름은 이 머신의 로컬 플레이어(호스트/Standalone)일 때만
    // 적용. 클라이언트 입장 시 서버의 GI는 호스트 이름이므로, 원격 클라이언트에
    // 호스트 이름을 덮어쓰지 않음.
    UWorld *WorldForCheck = GetWorld();
    const bool bIsLocalPlayer =
        WorldForCheck && (WorldForCheck->GetNetMode() == NM_Standalone ||
                          NewPlayer->IsLocalController());
    UBRGameInstance *GI = Cast<UBRGameInstance>(GetGameInstance());
    UE_LOG(LogTemp, Warning,
           TEXT("[로비이름] PostLogin | bIsLocalPlayer=%d | NetMode=%d | "
                "IsLocalController=%d"),
           bIsLocalPlayer ? 1 : 0,
           WorldForCheck ? (int32)WorldForCheck->GetNetMode() : -1,
           NewPlayer->IsLocalController() ? 1 : 0);

    if (bIsLocalPlayer && GI) {
      FString SavedPlayerName = GI->GetPlayerName();
      UE_LOG(LogTemp, Warning,
             TEXT("[로비이름] PostLogin | 로컬 플레이어 → "
                  "GI->GetPlayerName()='%s'"),
             *SavedPlayerName);
      if (!SavedPlayerName.IsEmpty()) {
        ...
        UE_LOG(
            LogTemp, Warning,
            TEXT("[로비이름] PostLogin | 로컬: BRPS->SetPlayerName('%s') 적용"),
            *PlayerName);
      }
    }
    // 원격 클라이언트: 이름을 "Player N"으로 덮어쓰지 않음. 빈/PC이름이면
    // 그대로 두고 ServerSetPlayerName 도착 시 실제 이름으로 갱신 → UI는 공란 후
    // PlayerName 표시
    if (!bIsLocalPlayer) {
      const FString CurrentName = BRPS->GetPlayerName();
      UE_LOG(LogTemp, Warning,
             TEXT("[로비이름] PostLogin | 원격 클라이언트: GetPlayerName() "
                  "유지 '%s' (ServerSetPlayerName 도착 시 갱신)"),
             *CurrentName);
    }

    // UserUID 설정 (GameInstance에서 가져오거나 생성)
    // 우선 기본 프로필 초기화
    FBaseUserInfo NewProfile;
    NewProfile.PlayerName = PlayerName;

    if (GI && bIsLocalPlayer) {
      // 로컬 플레이어는 GI 정보 그대로 사용
      NewProfile = GI->MyProfile;
    }

    // UID가 비어있으면 자동 생성 (예: Steam ID, 계정 ID 등)
    if (NewProfile.UserUID.IsEmpty()) {
      NewProfile.UserUID = FString::Printf(TEXT("Player_%d_%s"),
                                           BRGameState->PlayerArray.Num() - 1,
                                           *FDateTime::Now().ToString());
    }

    BRPS->SetPlayerProfile(NewProfile);
    UE_LOG(LogTemp, Warning,
           TEXT("[로비이름] PostLogin | SetPlayerProfile 완료 | Name='%s' "
                "UID='%s'"),
           *NewProfile.PlayerName, *NewProfile.UserUID);

    // 클라이언트 연결 확인 — 방 생성(ListenServer/Dedicated) 시에만 상세 로그,
    // Standalone은 최소 로그
    ...
    if (NetMode == NM_ListenServer || NetMode == NM_DedicatedServer) {
      ...
      UE_LOG(LogTemp, Warning, TEXT("[서버] 현재 인원: %d/%d"),
             BRGameState->PlayerArray.Num(), BRGameState->MaxPlayers);
      UE_LOG(LogTemp, Warning, TEXT("[서버] 네트워크 모드: %s"),
             *NetModeString);
      UE_LOG(LogTemp, Warning,
             TEXT("[서버] ✅ 리슨 서버 모드로 정상 실행 중입니다!"));
      UE_LOG(LogTemp, Warning,
             TEXT("[서버] 클라이언트 연결을 받을 수 있는 상태입니다."));
      if (GEngine) {
        FString JoinMsg = FString::Printf(
            TEXT("[서버] %s 입장! (인원: %d/%d)"), *PlayerName,
            BRGameState->PlayerArray.Num(), BRGameState->MaxPlayers);
      ...
      UE_LOG(
          LogTemp, Log,
          TEXT("[플레이어 입장] %s (Standalone — 방 생성 버튼으로 세션 생성)"),
          *PlayerName);
      UE_LOG(LogTemp, Log,
             TEXT("[플레이어 입장] 참고: 실제 방(세션)을 만들려면 'CreateRoom "
                  "[방이름]' 명령어 또는 방 생성 버튼을 사용하세요."));
    }

    // [보존] 방장 설정 — ListenServer/DedicatedServer에서만 적용 (Standalone은
    // 방 생성 버튼 누를 때만 방 생성)
    if (BRGameState->PlayerArray.Num() == 1 &&
        (NetMode == NM_ListenServer || NetMode == NM_DedicatedServer)) {
      UE_LOG(
          LogTemp, Log,
          TEXT("[플레이어 입장] 첫 번째 플레이어이므로 방장으로 설정됩니다."));
      BRPS->SetIsHost(true);
      // 방 제목을 서버에서 설정·복제하여 입장한 클라이언트도 "○○'s Game"으로
      // 동일하게 표시
      FString RoomTitleStr = PlayerName.IsEmpty()
                                 ? FString(TEXT("Host's Game"))
                                 : (PlayerName + TEXT("'s Game"));
      BRGameState->SetRoomTitle(RoomTitleStr);
    } else if (BRGameState->PlayerArray.Num() > 1 &&
               (NetMode == NM_ListenServer || NetMode == NM_DedicatedServer)) {
      // 입장한 클라이언트에게 RPC로 방 제목 즉시 전달 (복제 대기 없이 "○○'s
      // Game" 표시)
      FString RoomTitleStr =
          BRGameState->RoomTitle.IsEmpty()
      ...
      UE_LOG(LogTemp, Log,
             TEXT("[플레이어 입장] Standalone 모드 — 방 생성 버튼을 누르면 "
                  "방장이 됩니다."));
    }
    ...
    if (CurrentPlayerIndex == 0 || CurrentPlayerIndex % 2 == 0) {
      ...
      UE_LOG(LogTemp, Log, TEXT("[플레이어 역할] %s: 하체 역할 할당"),
             *PlayerName);
      ...
      if (BRGameState->PlayerArray.IsValidIndex(LowerBodyPlayerIndex)) {
        if (ABRPlayerState *LowerBodyPS = Cast<ABRPlayerState>(
                BRGameState->PlayerArray[LowerBodyPlayerIndex])) {
          // NewPlayer가 처음 접속하며 자동으로 배정받은 기본 Pawn을 가져옵니다.
          APawn *ProxyPawn = NewPlayer->GetPawn();
          if (ProxyPawn) {
            UE_LOG(LogTemp, Warning,
                   TEXT("[시스템] %s의 기존 Proxy Pawn(%s)을 삭제합니다."),
                   *PlayerName, *ProxyPawn->GetName());
            ProxyPawn->Destroy();
          }
          ...
          UE_LOG(LogTemp, Log,
                 TEXT("[플레이어 역할] %s: 상체 역할 할당 (하체 플레이어 "
                      "인덱스: %d)"),
          ...
          // -----------------------------------------------------------
          APlayerController *LowerBodyController =
              Cast<APlayerController>(LowerBodyPS->GetOwningController());
          if (LowerBodyController && UpperBodyClass) {
            APlayerCharacter *LowerChar =
                Cast<APlayerCharacter>(LowerBodyController->GetPawn());
            if (LowerChar) {
              ...
              AUpperBodyPawn *NewUpper = GetWorld()->SpawnActor<AUpperBodyPawn>(
                  UpperBodyClass, LowerChar->GetActorLocation(),
                  LowerChar->GetActorRotation(), SpawnParams);
              ...
              if (NewUpper) {
                ...
                UE_LOG(LogTemp, Log,
                       TEXT("[서버 생성] %s의 상체 Pawn이 생성되어 하체(Index "
                            "%d)에 부착되었습니다."),
                       *PlayerName, LowerBodyPlayerIndex);
              }
            }
...
void ABRGameMode::Logout(AController *Exiting) {
  ...
  if (ABRPlayerState *ExitingPS = Exiting->GetPlayerState<ABRPlayerState>()) {
    ...
    if (ABRGameState *BRGameState = GetGameState<ABRGameState>()) {
      UE_LOG(LogTemp, Log,
             TEXT("[플레이어 퇴장] %s가 방을 나갔습니다. (남은 인원: %d/%d)"),
             *ExitingPlayerName, BRGameState->PlayerArray.Num() - 1,
             BRGameState->MaxPlayers);
      ...
      if (!ExitingPS->bIsLowerBody && ExitingPS->ConnectedPlayerIndex >= 0) {
        if (ExitingPS->ConnectedPlayerIndex < BRGameState->PlayerArray.Num()) {
          if (ABRPlayerState *LowerBodyPS = Cast<ABRPlayerState>(
                  BRGameState->PlayerArray[ExitingPS->ConnectedPlayerIndex])) {
            LowerBodyPS->SetPlayerRole(true, -1); // 연결된 상체 없음
            UE_LOG(LogTemp, Log,
                   TEXT("[플레이어 퇴장] 하체 플레이어의 상체 연결 해제"));
          }
        }
      }
      // 나가는 플레이어가 하체인 경우, 연결된 상체 플레이어도 처리
      else if (ExitingPS->bIsLowerBody &&
               ExitingPS->ConnectedPlayerIndex >= 0) {
        if (ExitingPS->ConnectedPlayerIndex < BRGameState->PlayerArray.Num()) {
          if (ABRPlayerState *UpperBodyPS = Cast<ABRPlayerState>(
                  BRGameState->PlayerArray[ExitingPS->ConnectedPlayerIndex])) {
            // 상체 플레이어도 나가야 하거나, 다른 하체에 연결시킬 수 있음
            // 여기서는 단순히 연결 해제만 함
            UpperBodyPS->SetPlayerRole(false, -1);
            UE_LOG(LogTemp, Log,
                   TEXT("[플레이어 퇴장] 상체 플레이어의 하체 연결 해제"));
          }
        }
      }
      ...
      if (ExitingPS->bIsHost && BRGameState->PlayerArray.Num() > 1) {
        UE_LOG(
            LogTemp, Warning,
            TEXT(
                "[플레이어 퇴장] 방장이 나갔으므로 새로운 방장을 지정합니다."));
        // 다음 플레이어를 방장으로 설정
      ...
      if (ABRPlayerState *BRPS =
        ...
        if (i == 0) {
          ...
          if (LowerBodyPlayerIndex >= 0 &&
              LowerBodyPlayerIndex < BRGameState->PlayerArray.Num()) {
            if (ABRPlayerState *LowerBodyPS = Cast<ABRPlayerState>(
                    BRGameState->PlayerArray[LowerBodyPlayerIndex])) {
              BRPS->SetPlayerRole(false, LowerBodyPlayerIndex);
...
void ABRGameMode::ApplyRoleChangesForRandomTeams() {
  if (!HasAuthority() || !UpperBodyClass)
    return;
  ...
  UE_LOG(
      LogTemp, Warning,
      TEXT(
          "[랜덤 팀 적용] ApplyRoleChangesForRandomTeams 진입 (1.5초 타이머)"));

  ABRGameState *BRGameState = GetGameState<ABRGameState>();
  if (!BRGameState || BRGameState->PlayerArray.Num() < 2)
    return;

  // Seamless Travel 후 PlayerState가 초기화될 수 있으므로, 저장해 둔 팀/역할을
  // 복원
  ...
  Algo::Sort(SortedByTeam,
             [](const ABRPlayerState *A, const ABRPlayerState *B) {
  ...
  // 상체로 지정된 플레이어들의 기존(하체) Pawn만 먼저 제거 → 팀당 1개 하체
  // 몸통만 남김
  for (int32 TeamIndex = 0; TeamIndex < NumTeams; TeamIndex++) {
    ...
    {
      // PlayerState → Controller는 GetOwningController() 사용 (GetOwner()는
      // PlayerState에서 설정되지 않을 수 있음)
      APlayerController *UpperPC =
          Cast<APlayerController>(UpperPS->GetOwningController());
      if (UpperPC) {
  ...
  for (int32 TeamIndex = 0; TeamIndex < NumTeams; TeamIndex++) {
    ...
    if (LowerPS->bIsLowerBody == false || UpperPS->bIsLowerBody == true) {
      UE_LOG(LogTemp, Warning,
             TEXT("[랜덤 팀 적용] 팀 %d 역할 불일치 - 하체:%s 상체:%s, 스킵"),
             TeamIndex + 1, LowerPS->bIsLowerBody ? TEXT("Y") : TEXT("N"),
             UpperPS->bIsLowerBody ? TEXT("Y") : TEXT("N"));
      continue;
    }
    // PlayerState → Controller는 GetOwningController() 사용 (GetOwner()는
    // null일 수 있어 2팀 상체 등 빙의 실패 원인)
    APlayerController *LowerPC =
        Cast<APlayerController>(LowerPS->GetOwningController());
    APlayerController *UpperPC =
        Cast<APlayerController>(UpperPS->GetOwningController());
    if (!LowerPC || !UpperPC) {
      UE_LOG(LogTemp, Warning,
             TEXT("[랜덤 팀 적용] 팀 %d Controller 없음 - 하체PC:%s 상체PC:%s, "
                  "스킵"),
             TeamIndex + 1, LowerPC ? TEXT("O") : TEXT("X"),
             UpperPC ? TEXT("O") : TEXT("X"));
      continue;
    }
    ...
    if (!LowerChar || !IsValid(LowerChar)) {
      UE_LOG(LogTemp, Warning,
             TEXT("[랜덤 팀 적용] 팀 %d 하체 플레이어 %s의 Pawn 없음, 스킵"),
             TeamIndex + 1, *LowerPS->GetPlayerName());
      continue;
    }
    ...
    // 이 하체에 붙어 있던 기존 상체 Pawn 제거 (이터레이터 중 Destroy 방지를
    // 위해 수집 후 제거)
    ...
    if (NewUpper) {
      ...
      UE_LOG(LogTemp, Log, TEXT("[랜덤 팀 적용] 팀 %d: %s 상체 스폰 후 빙의"),
             TeamIndex + 1, *UpperPS->GetPlayerName());
    }
  }
}
...
void ABRGameMode::StartGame() {
  ...
  if (ABRGameState *BRGameState = GetGameState<ABRGameState>()) {
    ...
    if (bIsHost) {
      // 호스트는 자신이 준비하지 않아도 다른 모든 플레이어가 준비되었으면 시작
      // 가능
      ...
      if (bCanStart) {
        UE_LOG(LogTemp, Log,
               TEXT("[게임 시작] 호스트: 다른 모든 플레이어가 준비 완료 - 게임 "
                    "시작 가능"));
      } else {
        UE_LOG(LogTemp, Warning,
               TEXT("[게임 시작] 호스트: 조건 불만족 - 플레이어 수=%d/%d-%d, "
                    "호스트 제외 모든 준비=%s"),
               BRGameState->PlayerCount, BRGameState->MinPlayers,
               BRGameState->MaxPlayers,
               BRGameState->AreAllNonHostPlayersReady() ? TEXT("예")
                                                        : TEXT("아니오"));
      }
    } else {
      ...
      if (!bCanStart) {
        UE_LOG(LogTemp, Error,
               TEXT("[게임 시작] 실패: 게임을 시작할 수 없습니다."));
        UE_LOG(
            LogTemp, Warning,
            TEXT("[게임 시작] 조건 확인: 플레이어 수=%d/%d-%d, 모든 준비=%s"),
            BRGameState->PlayerCount, BRGameState->MinPlayers,
            BRGameState->MaxPlayers,
  ...
  if (bUseRandomMap && StageMapPaths.Num() > 0) {
    ...
    UE_LOG(LogTemp, Log, TEXT("[게임 시작] 기본 맵 사용: %s"),
           *SelectedMapPath);
  }

  UE_LOG(LogTemp, Log, TEXT("[게임 시작] 성공: 맵으로 이동 중... (%s)"),
         *SelectedMapPath);
  ...
  // Travel 직전에 역할 저장 (로비에서 선택한 1P/2P·팀 포함) + 게임 맵 로드 후
  // 상체/하체 Pawn 적용 예약
  if (UBRGameInstance *GI = Cast<UBRGameInstance>(GetGameInstance())) {
    if (ABRGameState *GS = GetGameState<ABRGameState>()) {
      GI->SavePendingRolesForTravel(GS);
      GI->SetPendingApplyRandomTeamRoles(
          true); // 랜덤이 아니어도 로비 역할(1P=하체, 2P=상체) 적용을 위해
                 // 플래그 설정
      UE_LOG(LogTemp, Warning,
             TEXT("[게임 시작] Travel 직전 역할 저장 완료 (GameMode), 게임 "
                  "맵에서 상체/하체 적용 예정"));
    }
  }
  ...
  if (bIsPIE) {
    UE_LOG(LogTemp, Warning,
           TEXT("[게임 시작] PIE 환경 감지 - 일반 ServerTravel 사용 (Seamless "
                "Travel 비활성화)"));
  }
  ...
  if (bShouldUseSeamlessTravel) {
    UE_LOG(LogTemp, Log, TEXT("[게임 시작] SeamlessTravel 호출: %s"),
           *TravelURL);
    ...
    World->ServerTravel(
        TravelURL, true); // 두 번째 파라미터는 bAbsolute (true = 절대 경로)
  } else {
    UE_LOG(LogTemp, Log,
           TEXT("[게임 시작] 일반 ServerTravel 호출: %s (PIE 모드: %s)"),
...
void ABRGameMode::EndPlay(const EEndPlayReason::Type EndPlayReason) {
  ...
  // 자동으로 정리되어야 하지만, PIE 종료 시 타이밍 문제로 인해 참조가 남을 수
  // 있습니다.
  //
  ...
  if (World && World->IsPlayInEditor()) {
    UE_LOG(LogTemp, Warning,
           TEXT("[GameMode] PIE 종료 - NavigationSystem은 World 파괴 시 "
                "자동으로 정리됩니다."));
    UE_LOG(LogTemp, Warning,
           TEXT("[GameMode] 참고: PIE 종료 시 NavigationSystem GC 경고는 "
                "Unreal Engine의 알려진 버그입니다."));
  }
}
...
void ABRGameMode::OnPlayerDied(ABaseCharacter *VictimCharacter) {
  ...
  UE_LOG(LogTemp, Warning, TEXT("[GameMode] 플레이어 사망 확인: %s"),
         *VictimCharacter->GetName());
