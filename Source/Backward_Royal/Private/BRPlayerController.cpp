...
ABRPlayerController::ABRPlayerController()
    : CurrentMenuWidget(nullptr), MainScreenWidget(nullptr) {
  // CheatManager 클래스 설정
...
void ABRPlayerController::BeginPlay() {
  ...
  if (CheatManager) {
    UE_LOG(LogTemp, Log,
           TEXT("[PlayerController] CheatManager 초기화 완료 - 콘솔 명령어 "
                "사용 가능"));
  } else {
    UE_LOG(LogTemp, Warning,
           TEXT("[PlayerController] CheatManager가 초기화되지 않았습니다."));
  }

  // 네트워크 연결 실패 델리게이트 바인딩
  if (UEngine *Engine = GEngine) {
    Engine->OnNetworkFailure().AddUObject(
        this, &ABRPlayerController::HandleNetworkFailure);
    UE_LOG(
        LogTemp, Log,
        TEXT("[PlayerController] 네트워크 실패 감지 델리게이트 바인딩 완료"));
  }
  ...
  if (UWorld *World = GetWorld()) {
    if (AGameModeBase *GameMode = World->GetAuthGameMode()) {
      if (ABRGameSession *GameSession =
        ...
        GameSession->OnCreateSessionComplete.AddDynamic(
            this, &ABRPlayerController::HandleCreateRoomComplete);
        UE_LOG(LogTemp, Log,
               TEXT("[PlayerController] GameSession 이벤트 바인딩 완료"));
      }
      // 클라이언트: 접속 시 내 프로필 정보를 서버에 전송 (타이머로 약간
      // 지연하여 안정성 확보)
      if (IsLocalController() && !HasAuthority()) {
        GetWorld()->GetTimerManager().SetTimerForNextTick([this]() {
          if (UBRGameInstance *GI = Cast<UBRGameInstance>(GetGameInstance())) {
            ServerSetProfile(GI->MyProfile);
            UE_LOG(LogTemp, Log,
                   TEXT("[BRPlayerController] 서버에 프로필 전송 요청: %s"),
                   *GI->MyProfile.PlayerName);
          }
        });
      }
    }
  ...
  // 클라이언트에서만 초기 UI 표시
  // PostLogin 완료 후 로비 판단하려면 짧은 지연 필요. EndPlay에서 타이머 해제 +
  // 람다 내 IsValid 검사로 open ?listen 크래시 방지.
  if (IsLocalController()) {
    UWorld *World = GetWorld();
    if (!World)
      return;
    World->GetTimerManager().SetTimer(
        BeginPlayUITimerHandle,
        [this]() {
          ...
          if (NetMode == NM_Client) {
            ...
            if (UNetDriver *NetDriver = W->GetNetDriver()) {
              if (UNetConnection *ServerConnection =
                      NetDriver->ServerConnection) {
                FString RemoteAddress =
                    ServerConnection->LowLevelGetRemoteAddress(true);
                UE_LOG(LogTemp, Warning, TEXT("[클라이언트] 서버 주소: %s"),
                       *RemoteAddress);
                UE_LOG(LogTemp, Warning,
                       TEXT("[클라이언트] 연결 상태: 연결됨"));
                ...
                UE_LOG(LogTemp, Error,
                       TEXT("[클라이언트] ServerConnection이 NULL입니다!"));
                if (GEngine) {
              ...
              UE_LOG(LogTemp, Error,
                     TEXT("[클라이언트] NetDriver가 NULL입니다!"));
            }

            // GameState 확인 (서버 데이터 복제 확인)
            if (ABRGameState *BRGameState = W->GetGameState<ABRGameState>()) {
              UE_LOG(LogTemp, Warning,
                     TEXT("[클라이언트] GameState 확인: 현재 인원 %d"),
                     BRGameState->PlayerArray.Num());
            } else {
              UE_LOG(LogTemp, Warning,
                     TEXT("[클라이언트] GameState가 아직 복제되지 않았습니다. "
                          "잠시 후 확인하세요."));
            }
          }

          // 세션이 활성화되어 있는지 확인 (ServerTravel 후 맵 재로드 시 세션이
          // 있을 수 있음)
          ...
          // ServerTravel 후 세션이 일시적으로 사라질 수 있지만, 플레이어 입장은
          // 유지됨
          ...
          // 세션이 있거나, (방 생성→ServerTravel 직후이며 플레이어 있음) 이면
          // 방 생성 완료 상태로 간주 Standalone 모드에서는 항상 main UI를
          // 표시하므로 bRoomCreated를 false로 설정
          bool bRoomCreated = false;
          if (NetMode != NM_Standalone) {
            // Standalone이 아닌 경우에만 방 생성 상태 확인
            bRoomCreated =
                bHasActiveSession || (bDidCreateRoomThenTravel && bHasPlayers);
          }

          if (GEngine) {
            FString DebugMsg = FString::Printf(
                TEXT("[BeginPlay] NetMode: %s, HasActiveSession: %s, Players: "
                     "%d, bDidCreateRoomThenTravel: %s"),
                NetMode == NM_Standalone     ? TEXT("Standalone")
                : NetMode == NM_ListenServer ? TEXT("ListenServer")
                : NetMode == NM_Client       ? TEXT("Client")
                                             : TEXT("Other"),
                bHasActiveSession ? TEXT("Yes") : TEXT("No"), PlayerCount,
                bDidCreateRoomThenTravel ? TEXT("Yes") : TEXT("No"));
            GEngine->AddOnScreenDebugMessage(-1, 5.0f, FColor::Cyan, DebugMsg);
          }

          UE_LOG(LogTemp, Warning,
                 TEXT("[PlayerController] BeginPlay UI 결정: NetMode=%s, "
                      "bHasActiveSession=%s, bHasPlayers=%s, "
                      "bDidCreateRoomThenTravel=%s"),
                 NetMode == NM_Standalone     ? TEXT("Standalone")
                 : NetMode == NM_ListenServer ? TEXT("ListenServer")
                 : NetMode == NM_Client       ? TEXT("Client")
                                              : TEXT("Other"),
          ...
          if (bRoomCreated && NetMode == NM_ListenServer) {
            // ListenServer 모드에서 세션이 활성화되어 있으면 정상적인 방 생성
            // 완료 상태
            UE_LOG(LogTemp, Log,
                   TEXT("[PlayerController] ListenServer 모드에서 "
                        "세션/플레이어 확인 - 로비로 이동"));
          }

          // 로비 표시 시 플래그 클리어 (다음 메인 복귀 시 엔트런스 표시용)
          if ((NetMode == NM_Client || NetMode == NM_ListenServer) &&
              (MainScreenWidget || LobbyMenuWidgetClass)) {
            if (UBRGameInstance *BRGI =
                    Cast<UBRGameInstance>(W->GetGameInstance())) {
          ...
          // MainScreenWidget이 설정되어 있으면 네트워크 모드에 따라 적절한
          // 메뉴로 전환
          if (MainScreenWidget && IsValid(MainScreenWidget)) {
            // Standalone 모드 처리
            if (NetMode == NM_Standalone) {
              // 방 생성 후 ServerTravel로 인한 재로드인 경우 LobbyMenu 표시
              // 세션이 없어도 플레이어가 있고 방 생성 플래그가 있으면 LobbyMenu
              // 표시 (ServerTravel 직후 NetMode가 아직 Standalone일 수 있지만,
              // 방 생성 후 재로드 상태면 LobbyMenu) 세션 체크는 제거 -
              // ServerTravel 직후 세션이 아직 초기화되지 않을 수 있음
              bool bShouldShowLobby = bDidCreateRoomThenTravel && bHasPlayers;

              UE_LOG(LogTemp, Warning,
                     TEXT("[PlayerController] Standalone 모드 UI 결정: "
                          "bShouldShowLobby=%s (bDidCreateRoomThenTravel=%s, "
                          "bHasPlayers=%s)"),
              ...
              if (bShouldShowLobby) {
                ...
                UE_LOG(LogTemp, Warning,
                       TEXT("[PlayerController] 초기 UI (LobbyMenu) 표시 - "
                            "Standalone 모드이지만 방 생성 후 재로드 상태"));
              } else {
                ...
                UE_LOG(LogTemp, Warning,
                       TEXT("[PlayerController] 초기 UI (EntranceMenu) 표시 - "
                            "Standalone 모드 (강제)"));
              }
              ...
              if (bHasActiveSession ||
                  (bDidCreateRoomThenTravel && bHasPlayers)) {
                // 세션이 있거나 방 생성 후 재로드 상태 - LobbyMenu 표시
                SetMainScreenToLobbyMenu();
                UE_LOG(LogTemp, Log,
                       TEXT("[PlayerController] 초기 UI (LobbyMenu) 표시 - "
                            "ListenServer 모드 (세션 있음)"));
              } else {
          ...
          // MainScreenWidget이 없으면 기존 방식대로 위젯 표시
            ...
            if (NetMode == NM_ListenServer) {
              ...
              if (bHasActiveSession ||
                ...
                if (LobbyMenuWidgetClass) {
                  ...
                  UE_LOG(LogTemp, Warning,
                         TEXT("[PlayerController] LobbyMenuWidgetClass가 "
                              "설정되지 않았습니다."));
                }
              } else {
                // 세션이 없음 - MainMenu 표시 (처음 실행 시)
                ...
                if (EntranceMenuWidgetClass) {
                  ...
                  UE_LOG(LogTemp, Warning,
                         TEXT("[PlayerController] EntranceMenuWidgetClass가 "
                              "설정되지 않았습니다."));
                }
              }
            }
            ...
            // Standalone 모드이면 EntranceMenu 표시 (단, 방 생성 후 재로드인
            // 경우는 예외)
            else {
              // 방 생성 후 ServerTravel로 인한 재로드인 경우 LobbyMenu 표시
              // 세션이 없어도 플레이어가 있고 방 생성 플래그가 있으면 LobbyMenu
              // 표시 세션 체크는 제거 - ServerTravel 직후 세션이 아직
              // 초기화되지 않을 수 있음
              bool bShouldShowLobby = bDidCreateRoomThenTravel && bHasPlayers;

              UE_LOG(LogTemp, Warning,
                     TEXT("[PlayerController] Standalone 모드 UI 결정 "
                          "(MainScreenWidget 없음): bShouldShowLobby=%s "
                          "(bDidCreateRoomThenTravel=%s, bHasPlayers=%s)"),
                     bShouldShowLobby ? TEXT("Yes") : TEXT("No"),
                     bDidCreateRoomThenTravel ? TEXT("Yes") : TEXT("No"),
                     bHasPlayers ? TEXT("Yes") : TEXT("No"));
              ...
              if (bShouldShowLobby) {
                ...
                if (EntranceMenuWidgetClass) {
                  ...
                  UE_LOG(
                      LogTemp, Warning,
                      TEXT(
                          "[PlayerController] EntranceMenuWidgetClass가 "
                          "설정되지 않았습니다. 블루프린트에서 설정해주세요."));
                }
              }
            }
          }
...
void ABRPlayerController::OnPossess(APawn *aPawn) {
  ...
  // Seamless Travel 후 게임 맵에서 GameMode BeginPlay가 호출되지 않을 수 있음 →
  // Possess 시점에 랜덤 팀 적용 예약 (폴백)
  if (HasAuthority() && aPawn) {
    if (UBRGameInstance *GI = Cast<UBRGameInstance>(GetGameInstance())) {
      if (GI->GetPendingApplyRandomTeamRoles()) {
        ABRGameMode *GM =
            GetWorld() ? GetWorld()->GetAuthGameMode<ABRGameMode>() : nullptr;
        if (GM) {
          FTimerHandle H;
          GetWorld()->GetTimerManager().SetTimer(
              H, GM, &ABRGameMode::ApplyRoleChangesForRandomTeams, 1.5f, false);
          UE_LOG(LogTemp, Log,
                 TEXT("[랜덤 팀 적용] OnPossess 폴백 - 1.5초 후 상체/하체 Pawn "
                      "적용 예정"));
        } else {
          UE_LOG(LogTemp, Warning,
                 TEXT("[랜덤 팀 적용] 게임 맵 GameMode가 ABRGameMode가 아님 - "
                      "상체/하체 적용이 되지 않을 수 있습니다."));
        }
      }
    }
  }
...
void ABRPlayerController::ClearUIForShutdown() {
  // PIE 종료 시 월드 참조 잔류 방지: GEngine/GameSession 델리게이트를 먼저
  // 끊는다.
  if (UWorld *World = GetWorld()) {
...
void ABRPlayerController::EndPlay(const EEndPlayReason::Type EndPlayReason) {
  ...
  // GetBRPlayerController → SetMainScreenWidget/CreateRoomWithPlayerName 호출
  // 시 PC가 이미 None이 되어 "Accessed None" 크래시가 나는 것을 줄이기 위함
  ClearUIForShutdown();
...
void ABRPlayerController::HandleNetworkFailure(
  ...
  if (NetDriver) {
    FString ServerAddress =
        NetDriver->ServerConnection
            ? NetDriver->ServerConnection->LowLevelGetRemoteAddress(true)
            : TEXT("Unknown");
  ...
  if (FailureType == ENetworkFailure::ConnectionTimeout ||
    ...
    UE_LOG(LogTemp, Error,
           TEXT("  2. 방화벽이 포트를 차단함 (포트 7777 확인)"));
...
void ABRPlayerController::CreateRoom(const FString &RoomName) {
  ...
  if (World) {
    if (UBRGameInstance *BRGI =
            Cast<UBRGameInstance>(World->GetGameInstance())) {
      BRGI->SetDidCreateRoomThenTravel(true);
      // CreateRoomWithPlayerName이 이미 CachedRoomTitle 설정한 경우 덮어쓰지
      // 않음. CreateRoom만 호출 시 RoomName으로 캐시
      if (BRGI->GetCachedRoomTitle().IsEmpty()) {
        BRGI->SetCachedRoomTitle(RoomName.IsEmpty() ? TEXT("Host's Game")
                                                    : RoomName);
      }
      UE_LOG(LogTemp, Error,
             TEXT("[방 생성] SetDidCreateRoomThenTravel 플래그 설정"));
    }
  }
  ...
  if (HasAuthority()) {
    ...
    if (World) {
      if (AGameModeBase *GameMode = World->GetAuthGameMode()) {
        UE_LOG(LogTemp, Error, TEXT("[방 생성] GameMode 발견: %s"),
               *GameMode->GetClass()->GetName());
        if (ABRGameSession *GameSession =
                Cast<ABRGameSession>(GameMode->GameSession)) {
          UE_LOG(LogTemp, Error,
                 TEXT("[방 생성] GameSession 발견! 세션 생성 요청 중..."));
          GameSession->CreateRoomSession(RoomName);
        } else {
          UE_LOG(LogTemp, Error,
                 TEXT("[방 생성] ❌ GameSession을 찾을 수 없습니다."));
          if (GameMode->GameSession) {
            UE_LOG(LogTemp, Error,
                   TEXT("[방 생성] GameMode->GameSession 타입: %s "
                        "(ABRGameSession이 아님)"),
                   *GameMode->GameSession->GetClass()->GetName());
          } else {
            UE_LOG(LogTemp, Error,
                   TEXT("[방 생성] GameMode->GameSession이 NULL입니다."));
          }
        }
      } else {
        UE_LOG(LogTemp, Error,
               TEXT("[방 생성] ❌ GameMode를 찾을 수 없습니다."));
      }
    }
  } else {
    UE_LOG(LogTemp, Error,
           TEXT("[방 생성] 클라이언트 권한 - 서버로 RPC 전송..."));
    // 클라이언트에서는 서버로 RPC 전송
    ServerCreateRoom(RoomName);
  }
}

void ABRPlayerController::CreateRoomWithPlayerName(const FString &RoomName,
                                                   const FString &PlayerName) {
  UE_LOG(LogTemp, Error, TEXT("========================================"));
  UE_LOG(LogTemp, Error, TEXT("[방 생성] CreateRoomWithPlayerName 호출됨!"));
  UE_LOG(LogTemp, Error, TEXT("[방 생성] 방 이름: %s, 플레이어 이름: %s"),
         *RoomName, *PlayerName);
  UE_LOG(LogTemp, Error, TEXT("========================================"));
  ...
  // GameInstance에 이름·방제목 저장 (ServerTravel 후 PostLogin에서 적용 →
  // UserInfo/로비 UI에 정상 반영)
  if (UWorld *World = GetWorld()) {
    if (UBRGameInstance *BRGI =
            Cast<UBRGameInstance>(World->GetGameInstance())) {
    ...
    // PIE 스킵 또는 ServerTravel 실패 시에도 즉시 표시되도록 GameState
    // RoomTitle 갱신
...
void ABRPlayerController::FindRooms() {
  ...
  if (GEngine) {
    GEngine->AddOnScreenDebugMessage(
        -1, 2.0f, FColor::Cyan, TEXT("[PlayerController] 방 찾기 요청..."));
  }
    ...
    if (UWorld *World = GetWorld()) {
      if (AGameModeBase *GameMode = World->GetAuthGameMode()) {
        if (ABRGameSession *GameSession =
          ...
          UE_LOG(LogTemp, Error,
                 TEXT("[방 찾기] 실패: GameSession을 찾을 수 없습니다."));
        ...
        UE_LOG(LogTemp, Error,
               TEXT("[방 찾기] 실패: GameMode를 찾을 수 없습니다."));
...
void ABRPlayerController::JoinRoom(int32 SessionIndex) {
  UE_LOG(LogTemp, Log, TEXT("[방 참가] 명령 실행: 세션 인덱스=%d"),
         SessionIndex);

  // 화면에 디버그 메시지 표시
  if (GEngine) {
    FString Message =
        FString::Printf(TEXT("[방 참가] 요청: 세션 인덱스 %d"), SessionIndex);
  ...
  if (NetMode == NM_ListenServer || NetMode == NM_DedicatedServer) {
    UE_LOG(LogTemp, Warning,
           TEXT("[방 참가] 호스트는 이미 서버이므로 다른 방에 참가할 수 "
                "없습니다."));
    if (GEngine) {
      GEngine->AddOnScreenDebugMessage(
          -1, 5.0f, FColor::Yellow,
          TEXT("[방 참가] 호스트는 다른 방에 참가할 수 없습니다!"));
    }
    return;
  }

  // [수정] 클라이언트에서 직접 로컬 GameSession을 통해 참가 시도
  // 기존에는 서버 RPC를 호출했으나, 세션 참가는 클라이언트 시스템에서
  // 이루어져야 함
  if (IsLocalController()) {
    UE_LOG(LogTemp, Log,
           TEXT("[방 참가] 로컬 컨트롤러: 직접 세션 참가 시도..."));
    ...
    if (BRGameSession) {
      UE_LOG(LogTemp, Log,
             TEXT("[방 참가] BRGameSession 발견, JoinSessionByIndex(%d) 호출"),
             SessionIndex);
      BRGameSession->JoinSessionByIndex(SessionIndex);
    } else {
      UE_LOG(LogTemp, Error,
             TEXT("[방 참가] 실패: ABRGameSession을 찾을 수 없습니다."));
      if (GEngine) {
        GEngine->AddOnScreenDebugMessage(
            -1, 5.0f, FColor::Red,
            TEXT("[방 참가] 실패: 세션 관리자를 찾을 수 없습니다!"));
      }
    }
  } else {
...
void ABRPlayerController::JoinRoomWithPlayerName(int32 SessionIndex,
                                                 const FString &PlayerName) {
  // Standalone 모드에서도 확인 가능하도록 화면에 큰 메시지 표시
  UE_LOG(LogTemp, Warning, TEXT("========================================"));
  UE_LOG(LogTemp, Warning,
         TEXT("[PlayerController] JoinRoomWithPlayerName 호출됨"));
  UE_LOG(LogTemp, Warning, TEXT("세션 인덱스: %d, 플레이어 이름: %s"),
         SessionIndex, *PlayerName);
  UE_LOG(LogTemp, Warning, TEXT("========================================"));

  // 화면에 디버그 메시지 표시 (Standalone 모드에서도 보이도록)
  if (GEngine) {
    FString Message =
        FString::Printf(TEXT(">>> [PlayerController] JoinRoomWithPlayerName: "
                             "세션 %d, 이름 %s <<<"),
                        SessionIndex, *PlayerName);
  ...
  // GameInstance에 이름 저장 (참가 후 PostLogin에서 적용 → UserInfo/로비 UI에
  // 정상 반영)
  if (UWorld *World = GetWorld()) {
    if (UBRGameInstance *BRGI =
            Cast<UBRGameInstance>(World->GetGameInstance())) {
...
void ABRPlayerController::ToggleReady() {
  ...
  if (ABRPlayerState *BRPS = GetPlayerState<ABRPlayerState>()) {
    ...
    UE_LOG(LogTemp, Error,
           TEXT("[준비 상태] 실패: PlayerState를 찾을 수 없습니다."));
  }
}
...
void ABRPlayerController::RandomTeams() {
  ...
  if (!World) {
    UE_LOG(LogTemp, Error,
           TEXT("[랜덤 팀 배정] 실패: World를 찾을 수 없습니다."));
    return;
  }
  ...
  if (HasAuthority()) {
    ...
    bCanUse = true;
    UE_LOG(LogTemp, Log,
           TEXT("[랜덤 팀 배정] 서버 권한 확인 완료, 팀 배정 시작..."));
  } else {
    // 클라이언트인 경우 방장인지 확인
    if (ABRPlayerState *BRPS = GetPlayerState<ABRPlayerState>()) {
      if (BRPS->bIsHost) {
        bCanUse = true;
        UE_LOG(LogTemp, Log,
               TEXT("[랜덤 팀 배정] 방장 권한 확인 완료, 팀 배정 시작..."));
      } else {
        UE_LOG(LogTemp, Warning,
               TEXT("[랜덤 팀 배정] 실패: 방장만 사용할 수 있습니다."));
      }
    } else {
      UE_LOG(LogTemp, Error,
             TEXT("[랜덤 팀 배정] 실패: PlayerState를 찾을 수 없습니다."));
    }
  }
...
void ABRPlayerController::ChangeTeam(int32 PlayerIndex, int32 TeamNumber) {
  UE_LOG(LogTemp, Log,
         TEXT("[팀 변경] 명령 실행: PlayerIndex=%d, TeamNumber=%d"),
  ...
  if (HasAuthority()) {
    ...
    bCanUse = true;
    UE_LOG(LogTemp, Log,
           TEXT("[팀 변경] 서버 권한 확인 완료, 팀 변경 요청 중..."));
  } else {
    // 클라이언트인 경우 방장인지 확인
    if (ABRPlayerState *BRPS = GetPlayerState<ABRPlayerState>()) {
      if (BRPS->bIsHost) {
        bCanUse = true;
        UE_LOG(LogTemp, Log,
               TEXT("[팀 변경] 방장 권한 확인 완료, 팀 변경 요청 중..."));
      } else {
        UE_LOG(LogTemp, Warning,
               TEXT("[팀 변경] 실패: 방장만 사용할 수 있습니다."));
      }
    } else {
      UE_LOG(LogTemp, Error,
             TEXT("[팀 변경] 실패: PlayerState를 찾을 수 없습니다."));
    }
  }
...
void ABRPlayerController::StartGame() {
  ...
  if (HasAuthority()) {
    ...
    bCanStart = true;
    UE_LOG(LogTemp, Log,
           TEXT("[게임 시작] 서버 권한 확인 완료, 게임 시작 요청 중..."));
  } else {
    // 클라이언트인 경우 방장인지 확인
    if (ABRPlayerState *BRPS = GetPlayerState<ABRPlayerState>()) {
      if (BRPS->bIsHost) {
        bCanStart = true;
        UE_LOG(LogTemp, Log,
               TEXT("[게임 시작] 방장 권한 확인 완료, 게임 시작 요청 중..."));
      } else {
        UE_LOG(LogTemp, Warning,
               TEXT("[게임 시작] 실패: 방장만 사용할 수 있습니다."));
      }
    } else {
      UE_LOG(LogTemp, Error,
             TEXT("[게임 시작] 실패: PlayerState를 찾을 수 없습니다."));
    }
  }
...
// 서버 RPC 함수들
void ABRPlayerController::ServerCreateRoom_Implementation(
    const FString &RoomName) {
  CreateRoom(RoomName);
}

void ABRPlayerController::ServerFindRooms_Implementation() { FindRooms(); }

void ABRPlayerController::ServerJoinRoom_Implementation(int32 SessionIndex) {
  JoinRoom(SessionIndex);
}

void ABRPlayerController::ServerToggleReady_Implementation() { ToggleReady(); }

void ABRPlayerController::ServerRequestRandomTeams_Implementation() {
  // 서버에서 직접 팀 배정 실행 (역할/팀 번호만 설정, 상체 스폰·빙의는 게임 맵
  // 로드 후 적용)
  if (ABRGameState *BRGameState = GetWorld()->GetGameState<ABRGameState>()) {
    UE_LOG(LogTemp, Log,
           TEXT("[랜덤 팀 배정] 서버에서 직접 실행: 총 %d명의 플레이어"),
           BRGameState->PlayerArray.Num());
    BRGameState->AssignRandomTeams();
    UBRGameInstance *GI =
        GetWorld() ? Cast<UBRGameInstance>(GetWorld()->GetGameInstance())
                   : nullptr;
    if (GI) {
      GI->SavePendingRolesForTravel(BRGameState);
      GI->SetPendingApplyRandomTeamRoles(true);
      UE_LOG(LogTemp, Warning,
             TEXT("[랜덤 팀 배정] 역할 저장 완료, 게임 맵 이동 시 상체/하체 "
                  "Pawn 적용 예약"));
    } else {
      UE_LOG(LogTemp, Error,
             TEXT("[랜덤 팀 배정] GameInstance 없음 - 역할 저장 스킵, "
                  "상체/하체 복원 실패 가능"));
    }
    UE_LOG(LogTemp, Log, TEXT("[랜덤 팀 배정] 완료"));
  } else {
    UE_LOG(LogTemp, Error,
           TEXT("[랜덤 팀 배정] 실패: GameState를 찾을 수 없습니다."));
  }
}

void ABRPlayerController::ServerRequestChangePlayerTeam_Implementation(
    int32 PlayerIndex, int32 NewTeamNumber) {
  // 서버에서 직접 팀 변경 실행
  if (ABRGameState *BRGameState = GetWorld()->GetGameState<ABRGameState>()) {
    if (PlayerIndex >= 0 && PlayerIndex < BRGameState->PlayerArray.Num()) {
      if (ABRPlayerState *TargetPS =
              Cast<ABRPlayerState>(BRGameState->PlayerArray[PlayerIndex])) {
        ...
        UE_LOG(LogTemp, Log, TEXT("[팀 변경] 서버에서 직접 실행: %s -> 팀 %d"),
               *PlayerName, NewTeamNumber);
      ...
      UE_LOG(LogTemp, Error, TEXT("[팀 변경] 실패: 잘못된 PlayerIndex (%d)"),
             PlayerIndex);
    }
  } else {
    UE_LOG(LogTemp, Error,
           TEXT("[팀 변경] 실패: GameState를 찾을 수 없습니다."));
  }
}
...
void ABRPlayerController::ServerSetPlayerName_Implementation(
    const FString &NewPlayerName) {
  UE_LOG(LogTemp, Warning,
         TEXT("[로비이름] ServerSetPlayerName 수신 | NewPlayerName='%s' | "
              "HasAuthority=%d"),
         *NewPlayerName, HasAuthority() ? 1 : 0);
  if (ABRPlayerState *BRPS = GetPlayerState<ABRPlayerState>()) {
    FString OldName = BRPS->GetPlayerName();
    FString OldUID = BRPS->MyProfile.UserUID;
    BRPS->SetPlayerNameString(NewPlayerName);
    UE_LOG(LogTemp, Warning,
           TEXT("[로비이름] ServerSetPlayerName 적용 | 이전 PlayerName='%s' "
                "UserUID='%s' → 새 PlayerName='%s'"),
           *OldName, *OldUID, *BRPS->GetPlayerName());
    // SetPlayerNameString 내부에서 NotifyUserInfoChanged() → UpdatePlayerList()
    // 호출됨
  }
}

void ABRPlayerController::ServerSetTeamNumber_Implementation(
    int32 NewTeamNumber) {
  if (ABRPlayerState *BRPS = GetPlayerState<ABRPlayerState>()) {
    BRPS->SetTeamNumber(NewTeamNumber);
    UE_LOG(LogTemp, Log, TEXT("[팀 번호 설정] 서버에서 설정: 팀 %d"),
           NewTeamNumber);
  }
}
...
void ABRPlayerController::ServerSetPlayerRole_Implementation(bool bLowerBody) {
  if (ABRPlayerState *BRPS = GetPlayerState<ABRPlayerState>()) {
    ...
    UE_LOG(LogTemp, Log, TEXT("[플레이어 역할 설정] 서버에서 설정: %s"),
           *RoleName);
  }
}

void ABRPlayerController::RequestAssignToLobbyTeam(int32 TeamIndex,
                                                   int32 SlotIndex) {
  ABRGameState *GS =
      GetWorld() ? GetWorld()->GetGameState<ABRGameState>() : nullptr;
  if (!GS)
    return;
  if (HasAuthority()) {
    ...
    if (PlayerIndex != INDEX_NONE &&
        GS->AssignPlayerToLobbyTeam(PlayerIndex, TeamIndex, SlotIndex)) {
      UE_LOG(LogTemp, Log, TEXT("[로비] 플레이어 %d -> 팀 %d 슬롯 %d 배치"),
             PlayerIndex, TeamIndex + 1, SlotIndex + 1);
    }
    return;
  }
...
void ABRPlayerController::RequestMoveToLobbyEntry(int32 TeamIndex,
                                                  int32 SlotIndex) {
  ABRGameState *GS =
      GetWorld() ? GetWorld()->GetGameState<ABRGameState>() : nullptr;
  if (!GS)
  ...
  if (HasAuthority()) {
    if (GS->MovePlayerToLobbyEntry(TeamIndex, SlotIndex)) {
      UE_LOG(LogTemp, Log, TEXT("[로비] 팀 %d 슬롯 %d -> Entry 이동"),
             TeamIndex + 1, SlotIndex + 1);
    }
    return;
  }
...
void ABRPlayerController::RequestMoveMyPlayerToLobbyEntry() {
  ...
  for (int32 TeamIndex = 0; TeamIndex < 4; ++TeamIndex) {
    for (int32 SlotIndex = 0; SlotIndex < 2; ++SlotIndex) {
      ...
      if (GS->LobbyTeamSlots.IsValidIndex(Flat) &&
          GS->LobbyTeamSlots[Flat] == PlayerIndex) {
...
void ABRPlayerController::ServerRequestAssignToLobbyTeam_Implementation(
    int32 TeamIndex, int32 SlotIndex) {
  ABRGameState *GS =
      GetWorld() ? GetWorld()->GetGameState<ABRGameState>() : nullptr;
  if (!GS || !HasAuthority())
    return;
  ...
  if (PlayerIndex != INDEX_NONE &&
      GS->AssignPlayerToLobbyTeam(PlayerIndex, TeamIndex, SlotIndex)) {
    UE_LOG(LogTemp, Log, TEXT("[로비] 서버: 플레이어 %d -> 팀 %d 슬롯 %d 배치"),
           PlayerIndex, TeamIndex + 1, SlotIndex + 1);
  }
}

void ABRPlayerController::ServerRequestMoveToLobbyEntry_Implementation(
    int32 TeamIndex, int32 SlotIndex) {
  ABRGameState *GS =
      GetWorld() ? GetWorld()->GetGameState<ABRGameState>() : nullptr;
  if (!GS || !HasAuthority())
    return;
  if (GS->MovePlayerToLobbyEntry(TeamIndex, SlotIndex)) {
    UE_LOG(LogTemp, Log, TEXT("[로비] 서버: 팀 %d 슬롯 %d -> Entry 이동"),
           TeamIndex + 1, SlotIndex + 1);
  }
}
...
void ABRPlayerController::ClientNotifyGameStarting_Implementation() {
  UE_LOG(
      LogTemp, Log,
      TEXT("[게임 시작] 클라이언트: 게임 시작 알림 수신 - 맵 이동 대기 중..."));

  // 클라이언트에서 입력을 일시적으로 중지 (선택사항)
  // 실제로는 ServerTravel이 자동으로 클라이언트를 따라오므로 특별한 처리가
  // 필요하지 않을 수 있습니다 하지만 로그를 남겨서 알림이 도착했는지 확인할 수
  // 있습니다
...
void ABRPlayerController::ClientReceiveRoomTitle_Implementation(
...
void ABRPlayerController::RequestRandomTeams() {
  ...
  // 서버에서 직접 실행 (역할/팀 번호만 설정, 상체 스폰·빙의는 게임 맵 로드 후
  // 적용)
  if (HasAuthority()) {
    if (ABRGameState *BRGameState = World->GetGameState<ABRGameState>()) {
      UE_LOG(LogTemp, Log,
             ...
             BRGameState->PlayerArray.Num());
      BRGameState->AssignRandomTeams();
      if (UBRGameInstance *GI =
              Cast<UBRGameInstance>(World->GetGameInstance())) {
        GI->SetPendingApplyRandomTeamRoles(true);
        UE_LOG(
            LogTemp, Log,
            TEXT("[랜덤 팀 배정] 게임 맵 이동 시 상체/하체 Pawn 적용 예약됨"));
      }
      UE_LOG(LogTemp, Log, TEXT("[랜덤 팀 배정] 완료"));
    } else {
      UE_LOG(LogTemp, Error,
    ...
    // 클라이언트에서 서버로 요청
    UE_LOG(LogTemp, Log,
           TEXT("[랜덤 팀 배정] 클라이언트에서 서버로 요청 전송..."));
    ServerRequestRandomTeams();
  }
}

void ABRPlayerController::RequestChangePlayerTeam(int32 PlayerIndex,
                                                  int32 NewTeamNumber) {
  UWorld *World = GetWorld();
  if (!World) {
    UE_LOG(LogTemp, Error, TEXT("[팀 변경] 실패: World를 찾을 수 없습니다."));
    ...
    if (ABRGameState *BRGameState = World->GetGameState<ABRGameState>()) {
      if (PlayerIndex >= 0 && PlayerIndex < BRGameState->PlayerArray.Num()) {
        if (ABRPlayerState *TargetPS =
          ...
          TargetPS->SetTeamNumber(NewTeamNumber);
          UE_LOG(LogTemp, Log,
                 TEXT("[팀 변경] 서버에서 직접 실행: %s의 팀이 %d에서 %d로 "
                      "변경되었습니다."),
                 *PlayerName, OldTeam, NewTeamNumber);
          BRGameState->OnTeamChanged.Broadcast();
        } else {
          UE_LOG(LogTemp, Error,
                 TEXT("[팀 변경] 실패: PlayerState 캐스팅 실패"));
        }
      } else {
        UE_LOG(LogTemp, Error,
               TEXT("[팀 변경] 실패: 잘못된 PlayerIndex (%d), 현재 플레이어 "
                    "수: %d"),
...
void ABRPlayerController::SetPlayerName(const FString &NewPlayerName) {
  UE_LOG(LogTemp, Warning,
         TEXT("[로비이름] SetPlayerName 호출 | NewPlayerName='%s' | "
              "HasAuthority=%d (0=클라이언트→서버 RPC 전송)"),
         *NewPlayerName, HasAuthority() ? 1 : 0);
  if (HasAuthority()) {
    if (ABRPlayerState *BRPS = GetPlayerState<ABRPlayerState>()) {
      BRPS->SetPlayerNameString(NewPlayerName);
...
void ABRPlayerController::RequestStartGame() {
  if (ABRPlayerState *BRPS = GetPlayerState<ABRPlayerState>()) {
    if (BRPS->bIsHost && HasAuthority()) {
      if (ABRGameMode *BRGameMode =
              GetWorld()->GetAuthGameMode<ABRGameMode>()) {
        if (ABRGameState *BRGameState =
                GetWorld()->GetGameState<ABRGameState>()) {
          UE_LOG(LogTemp, Log,
                 TEXT("[게임 시작] 조건 확인: 플레이어 수=%d/%d, 모든 플레이어 "
                      "준비=%s"),
                 BRGameState->PlayerCount, BRGameState->MaxPlayers,
                 BRGameState->AreAllPlayersReady() ? TEXT("예")
                                                   : TEXT("아니오"));
        }
        BRGameMode->StartGame();
      } else {
        UE_LOG(LogTemp, Error,
               TEXT("[게임 시작] 실패: GameMode를 찾을 수 없습니다."));
      }
    } else {
      // 클라이언트에서 서버로 요청
...
void ABRPlayerController::ShowRoomInfo() {
  ...
  FString ConnectionStatus =
      bIsConnected ? TEXT("연결됨") : TEXT("연결 안 됨 (Standalone)");
  ...
  if (NetMode == NM_Client) {
    UE_LOG(LogTemp, Log, TEXT("실제 연결 상태: %s"),
           bIsActuallyConnected ? TEXT("연결됨") : TEXT("연결 안 됨"));
  }
  UE_LOG(LogTemp, Log, TEXT("서버 권한: %s"),
         HasAuthority() ? TEXT("예 (서버)") : TEXT("아니오 (클라이언트)"));

  // PlayerArray와 PlayerCount 비교
  int32 ActualPlayerCount = BRGameState->PlayerArray.Num();
  UE_LOG(LogTemp, Log, TEXT("플레이어 수 (PlayerCount): %d / %d (최소: %d)"),
         BRGameState->PlayerCount, BRGameState->MaxPlayers,
         BRGameState->MinPlayers);
  UE_LOG(LogTemp, Log, TEXT("플레이어 수 (PlayerArray): %d명"),
         ActualPlayerCount);

  if (ActualPlayerCount != BRGameState->PlayerCount) {
    UE_LOG(LogTemp, Warning,
           TEXT("경고: PlayerCount(%d)와 PlayerArray(%d)가 일치하지 않습니다!"),
  ...
  UE_LOG(LogTemp, Log, TEXT("게임 시작 가능: %s"),
         BRGameState->bCanStartGame ? TEXT("예") : TEXT("아니오"));
  UE_LOG(LogTemp, Log, TEXT("내 상태 - 방장: %s, 준비: %s, 팀: %d"),
         LocalPS->bIsHost ? TEXT("예") : TEXT("아니오"),
         LocalPS->bIsReady ? TEXT("예") : TEXT("아니오"), LocalPS->TeamNumber);
  ...
  if (BRGameState->PlayerArray.Num() == 0) {
    ...
    if (!HasAuthority()) {
      UE_LOG(LogTemp, Warning,
             TEXT("클라이언트: 서버의 GameState가 아직 복제되지 않았을 수 "
                  "있습니다."));
    }
  }

  for (int32 i = 0; i < BRGameState->PlayerArray.Num(); i++) {
    if (ABRPlayerState *PS =
            Cast<ABRPlayerState>(BRGameState->PlayerArray[i])) {
      FString PlayerName = PS->GetPlayerName();
      if (PlayerName.IsEmpty()) {
        PlayerName = FString::Printf(TEXT("Player %d"), i + 1);
      }

      FString TeamText = PS->TeamNumber == 0
                             ? TEXT("팀 없음")
                             : FString::Printf(TEXT("팀 %d"), PS->TeamNumber);
      UE_LOG(LogTemp, Log, TEXT("[%d] %s - %s, 준비: %s, 방장: %s"), i,
             *PlayerName, *TeamText, PS->bIsReady ? TEXT("예") : TEXT("아니오"),
...
void ABRPlayerController::SetupRoleInput(bool bIsLower) {
  ...
  if (UEnhancedInputLocalPlayerSubsystem *Subsystem =
    ...
    UInputMappingContext *TargetContext =
        bIsLower ? LowerBodyContext : UpperBodyContext;
    if (TargetContext) {
...
void ABRPlayerController::ShowEntranceMenu() {
  if (EntranceMenuWidgetClass) {
    ...
    UE_LOG(LogTemp, Warning,
           TEXT("[PlayerController] EntranceMenuWidgetClass가 설정되지 "
                "않았습니다."));
  }
}
...
void ABRPlayerController::ShowJoinMenu() {
  if (JoinMenuWidgetClass) {
    ...
    UE_LOG(
        LogTemp, Warning,
        TEXT("[PlayerController] JoinMenuWidgetClass가 설정되지 않았습니다."));
  }
}
...
void ABRPlayerController::ShowLobbyMenu() {
  ...
  if (World && World->GetNetMode() == NM_Standalone) {
    UE_LOG(LogTemp, Warning,
           TEXT("[PlayerController] ShowLobbyMenu: Standalone 모드에서는 "
                "LobbyMenu를 표시하지 않습니다. EntranceMenu를 표시합니다."));
    ShowEntranceMenu();
    return;
  }
  ...
  if (LobbyMenuWidgetClass) {
    ...
    UE_LOG(
        LogTemp, Warning,
        TEXT("[PlayerController] LobbyMenuWidgetClass가 설정되지 않았습니다."));
  }
}

void ABRPlayerController::SetMainScreenWidget(UUserWidget *Widget) {
  MainScreenWidget = Widget;
  UE_LOG(LogTemp, Log, TEXT("[PlayerController] MainScreenWidget 설정: %s"),
...
void ABRPlayerController::ShowMainScreen() {
  if (MainScreenWidget && IsValid(MainScreenWidget)) {
    ...
    UE_LOG(
        LogTemp, Warning,
        TEXT(
            "[PlayerController] MainScreenWidgetClass가 설정되지 않았습니다."));
  }
}
...
void ABRPlayerController::SetMainScreenToEntranceMenu() {
  ...
  if (MainScreenWidget && IsValid(MainScreenWidget)) {
    UFunction *Function =
        MainScreenWidget->FindFunction(FName("SetMainScreenToEntranceMenu"));
    if (Function) {
      MainScreenWidget->ProcessEvent(Function, nullptr);
      UE_LOG(LogTemp, Log,
             TEXT("[PlayerController] SetMainScreenToEntranceMenu 호출 완료"));
      return;
    }
  }
  UE_LOG(LogTemp, Warning,
         TEXT("[PlayerController] SetMainScreenToEntranceMenu: "
              "MainScreenWidget이 없거나 함수를 찾을 수 없습니다."));
}

void ABRPlayerController::SetMainScreenToLobbyMenu() {
  UWorld *World = GetWorld();
  // 클라이언트가 로비 맵에 도착한 뒤 한 번 더 이름 전송 → 서버에
  // ServerSetPlayerName 반영 (맵 이동 전 RPC 유실 대비)
  if (World && World->GetNetMode() == NM_Client && IsLocalController()) {
    if (UBRGameInstance *BRGI =
      ...
      if (!Name.IsEmpty()) {
        SetPlayerName(Name);
        UE_LOG(LogTemp, Log,
               TEXT("[로비이름] 클라이언트 로비 진입 시 이름 재전송: '%s'"),
               *Name);
      }
    }
  }
  ...
  if (World && World->GetNetMode() == NM_Standalone) {
    ...
    // 플레이어가 있는지 확인 (세션 체크는 제거 - ServerTravel 직후 세션이 아직
    // 초기화되지 않을 수 있음)
    bool bHasPlayers = false;
    if (ABRGameState *BRGameState = World->GetGameState<ABRGameState>()) {
    ...
    if (!bIsRoomCreationReload || !bHasPlayers) {
      UE_LOG(LogTemp, Warning,
             TEXT("[PlayerController] SetMainScreenToLobbyMenu: Standalone "
                  "모드에서는 LobbyMenu를 표시하지 않습니다."));
      return;
    }
  }
  ...
  if (MainScreenWidget && IsValid(MainScreenWidget)) {
    UFunction *Function =
        MainScreenWidget->FindFunction(FName("SetMainScreenToLobbyMenu"));
    if (Function) {
      MainScreenWidget->ProcessEvent(Function, nullptr);
      UE_LOG(LogTemp, Log,
             TEXT("[PlayerController] SetMainScreenToLobbyMenu 호출 완료"));
      // LobbyMenu 전환 직후 OnPlayerListChanged 브로드캐스트 → 방 제목·플레이어
      // 목록 UI 즉시 갱신
      if (ABRGameState *BRGS = World->GetGameState<ABRGameState>()) {
  ...
  UE_LOG(LogTemp, Warning,
         TEXT("[PlayerController] SetMainScreenToLobbyMenu: MainScreenWidget이 "
              "없거나 함수를 찾을 수 없습니다."));
}

void ABRPlayerController::HandleCreateRoomComplete(bool bWasSuccessful) {
  UE_LOG(LogTemp, Log, TEXT("[PlayerController] 방 생성 완료: %s"),
         bWasSuccessful ? TEXT("성공") : TEXT("실패"));
  ...
  if (bWasSuccessful) {
    ...
    // Standalone 모드에서 방 생성 성공 시 ServerTravel이 호출되어 맵이 다시
    // 로드되므로 이 시점에서는 아직 Standalone 모드입니다. 맵이 다시 로드되면
    // BeginPlay에서 ListenServer 모드로 감지하여 LobbyMenu가 표시됩니다. 하지만
    // 이미 ListenServer 모드이거나 MainScreenWidget이 설정되어 있으면 즉시
    // LobbyMenu로 전환
    if (NetMode == NM_ListenServer) {
      ...
      if (MainScreenWidget && IsValid(MainScreenWidget)) {
        SetMainScreenToLobbyMenu();
        UE_LOG(LogTemp, Log,
               TEXT("[PlayerController] 방 생성 성공 - LobbyMenu로 전환 "
                    "(MainScreen)"));
      }
    ...
    // Standalone 모드에서는 ServerTravel이 호출되므로 여기서는 EntranceMenu
    // 완전히 제거 (맵이 다시 로드되면 BeginPlay에서 ListenServer 모드로
    // 감지하여 LobbyMenu가 표시됨)
    else if (NetMode == NM_Standalone) {
      // EntranceMenu 위젯 완전히 제거
      HideCurrentMenu();
      UE_LOG(LogTemp, Log,
             TEXT("[PlayerController] 방 생성 성공 - EntranceMenu 제거 (리슨 "
                  "서버로 전환 대기 중...)"));
      ...
      if (GEngine) {
        GEngine->AddOnScreenDebugMessage(
            ...
            TEXT("맵 재로드 중... 잠시만 기다려주세요."));
      }
    }
  } else {
    UE_LOG(LogTemp, Error,
           TEXT("[PlayerController] 방 생성 실패 - EntranceMenu 유지"));
...
void ABRPlayerController::LeaveRoom() {
  ...
  // 클라이언트: 서버 연결을 끊으면 서버에서 Logout(Exiting)이 호출되어
  // PlayerArray에서 제거됨
  if (NetMode == NM_Client) {
  ...
  // 호스트(ListenServer): 세션을 종료하고 메인 맵으로 이동 (모든 클라이언트도
  // 함께 이동)
  if (NetMode == NM_ListenServer) {
    if (AGameModeBase *GameMode = World->GetAuthGameMode()) {
      if (ABRGameSession *GameSession =
        ...
        UE_LOG(LogTemp, Warning,
               TEXT("[방 나가기] 호스트: BRGameSession을 찾을 수 없습니다."));
      }
    }
    return;
  }
...
void ABRPlayerController::ShowMenuWidget(TSubclassOf<UUserWidget> WidgetClass) {
  ...
  if (MainScreenWidget && IsValid(MainScreenWidget)) {
    MainScreenWidget->SetVisibility(ESlateVisibility::Collapsed);
    UE_LOG(LogTemp, Log,
           TEXT("[PlayerController] WBP_MainScreen 숨김 (HUD에서 관리)"));
  }
  ...
  if (WidgetClass) {
    ...
    if (CurrentMenuWidget) {
      ...
      if (CurrentMenuWidget->GetOwningPlayer() == nullptr) {
        ...
        UE_LOG(LogTemp, Warning,
               TEXT("[PlayerController] 위젯 OwningPlayer가 null이었습니다. "
                    "수동으로 설정했습니다."));
      }
    } else {
...
void ABRPlayerController::SubmitCustomizationToServer() {
  ...
  if (GI && PS) {
    ...
    UE_LOG(LogTemp, Log,
           TEXT("커스터마이징 정보를 서버로 전송했습니다. HeadID: %d"),
           LocalData.HeadID);
  }
}