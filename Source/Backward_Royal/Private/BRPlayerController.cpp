...
void ABRPlayerController::BeginPlay()
{
	...
	if (IsLocalController())
	{
		...
		if (World)
		{
			// 리슨 실패/맵 리로드 시 람다에서 GetWorld() 호출이 크래시할 수 있음 → 월드/컨트롤러를 Weak로 캡처해 람다 내부에서는 GetWorld() 호출 안 함
			TWeakObjectPtr<UWorld> WeakWorld(World);
			TWeakObjectPtr<ABRPlayerController> WeakPC(this);
			World->GetTimerManager().SetTimer(SubmitTimer, [WeakWorld, WeakPC]()
				{
					...
					ABRPlayerController* PC = WeakPC.Get();
					if (PC->GetPlayerState<ABRPlayerState>())
					{
						PC->SubmitCustomizationToServer();
					}
					else
					{
						FTimerHandle RetryHandle;
						W->GetTimerManager().SetTimer(RetryHandle, PC, &ABRPlayerController::SubmitCustomizationToServer, 0.5f, false);
					}
				}, 0.2f, false);
		}
	}
