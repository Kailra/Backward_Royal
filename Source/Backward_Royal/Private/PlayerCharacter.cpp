...
#define LOG_PLAYER(Verbosity, Format, ...)                                     \
  UE_LOG(LogPlayerChar, Verbosity, TEXT("%s - %s"), *FString(__FUNCTION__),    \
...
APlayerCharacter::APlayerCharacter() {
  ...
  // 1. 카메라 설정
  RearCameraBoom =
      CreateDefaultSubobject<USpringArmComponent>(TEXT("RearCameraBoom"));
  ...
  // 2. 상체 부착 지점
  HeadMountPoint =
      CreateDefaultSubobject<USceneComponent>(TEXT("HeadMountPoint"));
...
void APlayerCharacter::BeginPlay() {
  if (StaminaComp) {
    StaminaComp->OnStaminaChanged.AddDynamic(
        this, &APlayerCharacter::HandleStaminaChanged);
    StaminaComp->OnSprintStateChanged.AddDynamic(
        this, &APlayerCharacter::HandleSprintStateChanged);
  }
...
void APlayerCharacter::Move(const FInputActionValue &Value) {
  ...
  if (Controller != nullptr) {
    ...
    FVector IntentDir =
        (CameraForward * MovementVector.Y + CameraRight * MovementVector.X)
            .GetSafeNormal();

    // 2. 내적(Dot Product) 계산
    // 캐릭터의 정면(GetActorForwardVector)과 이동하려는 방향(IntentDir)을 비교
    // 결과가 양수(+)면 캐릭터가 보는 방향으로 걷는 것이고, 음수(-)면 뒷걸음질
    // 치는 것
    ...
    // 현재 캐릭터는 카메라 등 뒤를 보고 있으므로, S키를 눌러 카메라 쪽으로 올
    // 때가 여기에 해당됨
    if (Dot > 0.1f) {
...
void APlayerCharacter::Restart() { Super::Restart(); }
...
void APlayerCharacter::OnRep_PlayerState() {
  ...
  if (MyPS) {
    // 1-1. 내 커마 정보가 오면 알려줘
    MyPS->OnCustomizationDataChanged.AddDynamic(
        this, &APlayerCharacter::TryApplyCustomization);

    // 1-2. 내 역할(상/하체)이나 파트너가 정해지면 알려줘
    // (기존 코드에 OnPlayerRoleChanged 델리게이트가 이미 있다고 가정)
    MyPS->OnPlayerRoleChanged.AddDynamic(
        this, &APlayerCharacter::BindToPartnerPlayerState);
...
void APlayerCharacter::TryApplyCustomization() {
  ...
  if (!bUpperBodyApplied && UpperPS &&
      UpperPS->MyProfile.UserCustomData.HeadID != 0) {
    ApplyMeshFromID(EArmorSlot::Head, UpperPS->MyProfile.UserCustomData.HeadID);
    ApplyMeshFromID(EArmorSlot::Chest,
                    UpperPS->MyProfile.UserCustomData.ChestID);
    ApplyMeshFromID(EArmorSlot::Hands,
                    UpperPS->MyProfile.UserCustomData.HandID);
  ...
  if (!bLowerBodyApplied && LowerPS &&
      LowerPS->MyProfile.UserCustomData.LegID != 0) {
    ApplyMeshFromID(EArmorSlot::Legs, LowerPS->MyProfile.UserCustomData.LegID);
    ApplyMeshFromID(EArmorSlot::Feet, LowerPS->MyProfile.UserCustomData.FootID);
...
void APlayerCharacter::BindToPartnerPlayerState(bool bIsLowerBody) {
  ...
  if (!MyPS) {
    ...
    GetWorld()->GetTimerManager().SetTimer(
        TimerHandle_RetryBindPartner,
        [this, bIsLowerBody]() { BindToPartnerPlayerState(bIsLowerBody); },
        0.5f, false);
    return;
  }
  ...
  ABRPlayerState *PartnerPS =
      Cast<ABRPlayerState>(GS->PlayerArray[MyPS->ConnectedPlayerIndex]);
  ...
  PartnerPS->OnCustomizationDataChanged.RemoveDynamic(
      this, &APlayerCharacter::TryApplyCustomization);
  PartnerPS->OnCustomizationDataChanged.AddDynamic(
      this, &APlayerCharacter::TryApplyCustomization);

  bBoundToPartner = true;
  LOG_PLAYER(Display, TEXT("Bound to Partner Success: %s"),
             *PartnerPS->GetPlayerName());
...
void APlayerCharacter::ApplyMeshFromID(EArmorSlot Slot, int32 MeshID) {
  ...
  if (!ArmorDT) {
    LOG_PLAYER(Error,
               TEXT("ArmorDataTable Not Found in GameInstance ConfigMap!"));
    return;
  }
